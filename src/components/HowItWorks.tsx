...
const HowItWorks = () => {
  return (
    <section id="how-it-works" className="py-16 md:py-32 relative section-dark overflow-hidden">
      {/* Grid pattern */}
        ...
        <div className="grid md:grid-cols-3 gap-4 md:gap-8 max-w-6xl mx-auto">
          {steps.map((step, index) => (
            ...
            >
              ...
              <div className="holo-border-dark p-4 md:p-8 h-full">
                {/* Number & Icon Row */}
                <div className="flex items-center justify-between mb-4 md:mb-8">
                  <span className="text-3xl md:text-6xl font-serif text-transparent bg-clip-text bg-gradient-to-br from-holo-1 via-holo-2 to-holo-3 opacity-60">
                    {step.number}
                  </span>
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-dark-card border border-dark-border flex items-center justify-center">
                    <step.icon className="w-4 h-4 md:w-5 md:h-5 text-holo-2" />
                  </div>
                </div>

                {/* Content */}
                <h3 className="text-base md:text-xl font-serif text-dark-fg mb-2 md:mb-3">
                  {step.title}
                </h3>
                <p className="text-dark-muted leading-relaxed font-sans text-xs md:text-sm">
                  {step.description}
                </p>
              </div>
            </motion.div>
